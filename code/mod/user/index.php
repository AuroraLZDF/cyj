...
class index
{
    ...
    public function loginFunc()
    {
        ...
        if (!$param['hashKey']) {
            $GLOBALS['tpl']->display('oa/user/login.tpl');
        } else {
            $rules = array(
            ...
            //mysql exec()异常
            $ip = getonlineip();
            $sql = "insert into login_log (`uid`,`ip`) values ('{$result['uid']}', '{$ip}')";
            if(!$this->mysql->execute($sql)){
                show_message('登录日志记录异常', '', 2);
            }*/

            webheader('/oa/index/index');
        }
    }
